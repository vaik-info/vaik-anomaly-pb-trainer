...
def draw_image(good_images, good_inf_images, good_mses, anomaly_images, anomaly_inf_images, anomaly_mses, output_dir_path, rescale=255.):
    ...
    for good_index, good_inf_image in enumerate(good_inf_images):
        ...
        good_inf_image.save(os.path.join(output_sub_dir_path, f'{good_mse_mean_list[good_index]:.4f}_inf.png'))
        good_image = Image.fromarray(good_images[good_index])
        good_image.save(os.path.join(output_sub_dir_path, f'{good_mse_mean_list[good_index]:.4f}_raw.png'))
    ...
    for anomaly_index, anomaly_inf_image in enumerate(anomaly_inf_images):
        ...
        anomaly_image = Image.fromarray(anomaly_images[anomaly_index])
        anomaly_image.save(os.path.join(output_sub_dir_path, f'{anomaly_mse_mean_list[anomaly_index]:.4f}_raw.png'))
