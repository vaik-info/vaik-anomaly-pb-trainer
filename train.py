...
def train(train_image_dir_path, test_good_image_dir_path, test_anomaly_image_dir_path,
    ...
    for epoch in range(epoch_size):
        with tqdm.tqdm(range(step_size), unit="steps") as monitor_tqdm:
            ...
            # valid
            train_loss, train_mse, train_generated_images = test_step(train_good_data[0], train_good_data[1])
            val_loss, val_mse, val_generated_images = test_step(valid_good_data[0], valid_good_data[1])
            print(f'loss:{float(train_loss):.4f}, val_loss:{float(val_loss):.4f}, mse:{float(train_mse):.4f}, val_mse:{float(val_mse):.4f}')
